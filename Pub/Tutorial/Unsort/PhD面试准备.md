...
模型上：
1. 二进制中的模型，不应该只追求性能先进，更需要能够挖掘出结构关系。但同时预训练模型确实能力很强。因此将LLMs模型和结构信息结合是一个可以研究的方向。具体来说，可以通过任务设计、损失函数设计、评价指标设计这几种方式，尽量引导模型提取结构信息。这就和尹老师之前的工作十分的类似，palmTree，JTrans。palmTree采用了预测滑动窗口内的数据流图、控制流图作为任务。JTrans采用了预测跳转信息作为任务。两者都是预测一个函数内的结构信息。palmTree采用的是对指令进行采样。然而其实还有其他信息可以使用，函数间的调用信息、文件间的调用信息，都可能用于对二进制进行语义提取。
2. 数据方面。现在的模型主要用到的数据就是汇编码，和其相应的结构跳转信息。那么实际上可以用到的信息还不止这些。如引入中间语言作为中介，也可以引入高级语言作为辅助信息。
3. 从下游任务上看。还存在一些下游任务可以进一步扩展，如从汇编反编译到高级语言。下游任务：代码克隆检测、恶意代码检测、相似漏洞挖掘，补丁对比分析，逆向工程辅助。
