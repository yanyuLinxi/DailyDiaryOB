...
模型上：
1. 二进制中的模型，不应该只追求性能先进，更需要能够挖掘出结构关系。但同时预训练模型确实能力很强。因此将LLMs模型和结构信息结合是一个可以研究的方向。具体来说，可以通过任务设计、损失函数设计、评价指标设计这几种方式，尽量引导模型提取结构信息。这就和尹老师之前的工作十分的类似，palmTree，JTrans。palmTree采用了预测滑动窗口内的数据流图、控制流图作为任务。JTrans采用了预测跳转信息作为任务。两者都是预测一个函数内的结构信息。palmTree采用的是对指令进行采样。然而其实还有其他信息可以使用，函数内部有
